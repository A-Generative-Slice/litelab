...
const lightingProducts = [
  ...
  { id: "busbar", name: "BusBar LED", category: "Infrastructure", type: "Power Track", watt: 15, lumens: 1200, efficiency: 80, features: "Modular power rail system with integrated LED" },
  { id: "highbay", name: "Industrial High Bay", category: "Infrastructure", type: "Heavy Duty", watt: 150, lumens: 18000, efficiency: 120, features: "High-ceiling industrial lighting, IP66" },
...
function setupEventListeners() {
  ...
  const refreshThree = document.getElementById("refreshThree");
  ...
  if (refreshThree) {
    refreshThree.addEventListener("click", () => {
      if (threeRenderer) {
        resizeThreePreview();
        updateThreeScene();
...
function updateROI(product) {
  ...
  // Handle call as event listener or missing product
  if (!product || product instanceof Event) {
    const lightId = document.getElementById("lightSelect").value;
    product = lightingProducts.find(p => p.id === lightId);
  }
        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              ...
              return `${label}: (${context.parsed.x}W, ${context.parsed.y} lm/W)`;
            }
          }
        }
      },
...
function initializeWorkspace() {
  ...
  // Don't resize or init 3D if hidden (will have 0 size)
  if (document.getElementById("workspace").classList.contains("active")) {
    resizeWorkspaceCanvas();
    initializeThreePreview();
    renderWorkspace();

    // Extra force render for 3D
    setTimeout(() => {
      if (threeRenderer) {
        resizeThreePreview();
        updateThreeScene();
...
function initializeThreePreview() {
  ...
  console.log("=== 3D Workspace Init ===");
  if (!container) return;

  if (!window.THREE || !window.OrbitControls) {
    console.warn("3D System not ready. Waiting...");
    setTimeout(initializeThreePreview, 200);
    return;
  }
  ...
  try {
    ...
    animateThree();
    console.log("3D Workspace Rendering Active");
  } catch (error) {
...
function updateThreeScene() {
  ...
  // Clear scene elements if room size changed
  if (lastThreeRoomDim.w !== roomWidth || lastThreeRoomDim.d !== roomDepth) {
    if (threeRoom) threeScene.remove(threeRoom);

    // Create an "Interior" Room (Visible from outside looking in)
    const roomGeometry = new THREE.BoxGeometry(roomWidth, roomHeight, roomDepth);
    const roomMaterial = new THREE.MeshStandardMaterial({
      color: 0x18181b, // Zinc-900
      metalness: 0.2,
      roughness: 0.8,
      side: THREE.BackSide // Allows looking through exterior walls into the interior
    });
    threeRoom = new THREE.Mesh(roomGeometry, roomMaterial);
    threeRoom.position.y = roomHeight / 2;
    threeRoom.receiveShadow = true;
    threeScene.add(threeRoom);

    // Floor Mesh (Shows light throw)
    ...
    lastThreeRoomDim = { w: roomWidth, d: roomDepth };

    // Auto-center camera target to the room center
    if (threeControls) {
      threeControls.target.set(0, roomHeight / 4, 0);
      threeControls.update();
    }
  }

  // Clear fixtures and lights
  threeLightsGroup.clear();

  if (workspaceState.fixtures.length === 0) {
  ...
  workspaceState.fixtures.forEach(fixture => {
    ...
    // Adjust logic based on lumen
    const intensityFactor = fixture.lumens / 2000;
    bulbMaterial.emissiveIntensity = Math.min(6, 2 + intensityFactor);

    const bulb = new THREE.Mesh(bulbGeometry, bulbMaterial);

    // 2. The Actual PointLight
    const lightIntensity = Math.min(12, 2.0 + (fixture.lumens / 600));
    const pointLight = new THREE.PointLight(0xf97316, lightIntensity, roomWidth * 2);
    pointLight.castShadow = true;
    pointLight.shadow.bias = -0.001; // Help with shadow acne
    ...
    threeLightsGroup.add(fixtureGroup);
  });
...
function initializeResizers() {
  const leftResizer = document.querySelector(".resizer[data-target='left']");
  const rightResizer = document.querySelector(".resizer[data-target='right']");
  const leftPanel = document.getElementById("fixtureSidebar");
  const rightPanel = document.getElementById("summarySidebar");

  if (!leftResizer || !rightResizer || !leftPanel || !rightPanel) return;
  ...
  leftResizer.addEventListener("mousedown", (e) => initResize(e, leftPanel, true));
  rightResizer.addEventListener("mousedown", (e) => initResize(e, rightPanel, false));
}
